...
> **実機検証済み（2026-08-16）**：LED・チューニングの状態はデバイス側で保持され、
> **ホスト（macOS）とのHID接続を閉じても変化しない**。すなわちCLIツールのように
> 「接続→1レポート送信→即切断」という一回限りの短命プロセスでも、
> コマンドの効果自体は正しく反映され、その後も持続する。目視確認の際に
> 「一瞬しか変化して見えない」場合は、コマンドが効いていないのではなく、
> プロセスの起動・終了が速すぎて視認しにくいだけの可能性が高い
> （`docs/implementation-plan.md` の検証ログ参照）。
